        ...
        where TSelf : JsonPropertyBase<TSelf>
{
		...
		public TSelf Converter (Type converterType)
		{
			try{
					var converter = Activator.CreateInstance (converterType) as IJsonConverter;
					if (converter == null) {
							throw (new Exception ("JsonFormat类型不正确"));
					}
					this.m_converter = converter;
					return (TSelf)this;
			}
			...
			{
				Logger.Error.Write("Create[{0}][{1}][{2}][{3}]",converterType.Name,ee.Message,ee.StackTrace,ee.InnerException.ToString());
        ...
        where TProperty : class
{
		...
		private class ContractConverter : IJsonConverter
		{
				...
				{
						return JsonSerializer.DeserializeObject<TProperty> ((JsonObject)value);
				}
		}
}
