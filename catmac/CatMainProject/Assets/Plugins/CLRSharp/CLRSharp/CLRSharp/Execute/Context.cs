...
namespace CLRSharp
{
    ...
    public class ThreadContext
    {
        ...
        Queue<StackFrame> stackPool = new Queue<StackFrame>();

        public void CacheStackFrame(StackFrame sf)
        {
            stackPool.Enqueue(sf);
        }

        public StackFrame GetCacheStackFrame(string name, bool isStatic)
        {
            StackFrame stack;
            if (stackPool.Count > 0)
            {
                stack = stackPool.Dequeue();
                stack.Reset(name, isStatic);
            }
            else
            {
                stack = new StackFrame(name, isStatic);
            }
            return stack;
        }
        ...
        public object ExecuteFunc(IMethod_Sharp method, object _this, object[] _params)
        {
            ...
            StackFrame stack = GetCacheStackFrame(method.Name, method.isStatic) ;
